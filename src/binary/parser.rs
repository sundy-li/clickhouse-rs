...
impl<'a, T: Read> Parser<T> {
    ...
    pub(crate) fn parse_packet(&mut self, hello: &Option<HelloRequest>, compress: bool) -> Result<Packet> {
        ...
        match packet {
            ...
            protocols::CLIENT_DATA | protocols::CLIENT_SCALAR => Ok(self.parse_data(packet == protocols::CLIENT_SCALAR, compress)?),
    ...
    fn parse_data(&mut self, _scalar: bool, compress: bool) -> Result<Packet> {
        ...
        let block = Block::load(&mut self.reader, self.tz.clone(), compress)?;
